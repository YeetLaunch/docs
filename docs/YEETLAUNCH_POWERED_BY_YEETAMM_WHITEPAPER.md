...
| 5 | Creator rug is impossible | LP is **30% permanently locked / 70% vested** (48h cliff, then 5%/day over 14 days); AMM mode retains 20 bps of every trade in-pool. The creator cannot pull liquidity. | §9.1, §9.4 |

Read together: entering a market is always allowed and cheap; exiting is always allowed, but (a) unprofitable to wash-pump, (b) unable to force or exploit graduation, and (c) throttled in raw speed — while the liquidity itself is locked and vested so the creator can never withdraw it out from under holders. Every one of these layers is exercised by the 87 Rust unit tests and the adversarial QA evidence proofs, including the explicit *no-profitable-round-trip* and *`k`-never-decreases* guards.
...
- `ag/MAINNET_READINESS_REPORT_2026-04-02.md`
